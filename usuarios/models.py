...
class Usuario(User):
    ...
    codigo_postal = models.IntegerField('Código postal', validators=[MinLengthValidator(5, 'El código postal debe contener cinco dígitos.'), MaxLengthValidator(5, 'El código postal debe contener cinco dígitos.') , solo_numeros])
    estado = models.ForeignKey(Estado, verbose_name='Estado', on_delete=models.CASCADE)
    municipio = models.ForeignKey(Municipio, verbose_name='Municipio', on_delete=models.CASCADE)
    localiad = models.ForeignKey(Localidad, verbose_name='Localidad', on_delete=models.CASCADE)
    correo = models.EmailField('Correo', max_length=50)
    telefono = models.CharField('Teléfono', max_length=10, validators=[MinLengthValidator(10, 'El teléfono debe contener diez dígitos.'), ])
