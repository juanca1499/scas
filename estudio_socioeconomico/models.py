...
class EstudioSocioeconomico(models.Model):
    folio = models.CharField("Folio",max_length=9, primary_key=True)
    fecha_actual = models.DateField("Fecha")
    credencial = models.FileField("Credencial INE/IFE", upload_to='credenciales_estudio/', validators=[validador_archivo])
    comprobante_domicilio = models.FileField("Comprobante de domicilio", upload_to='comprobantes/', validators=[validador_archivo])
    ...
    jefe_familia = models.BooleanField("¿Es jefe de familia?", default=False)
    ...
    otros_caracteristicas_casa = models.CharField("Otro", max_length=60)
    ...
    servicio_salud = models.ForeignKey(ServicioSalud, verbose_name="Ocupacion", on_delete=models.CASCADE)
    ...
    enfermedad_cardipatias = models.BooleanField("Cardiopatías", default=False)
    ...
    enfermedad_otro = models.CharField("otro (Especifique)", max_length=60)
    receta = models.FileField("Receta en caso de ser necesario", upload_to='recetas/', validators=[validador_archivo],null=True)
