...
class EstudioSocioeconomico(models.Model): 
    ...
    comprobante_domicilio = models.FileField(
        "Comprobante de domicilio", upload_to='estudio/comprobante/', validators=[validador_archivo])
    solicitud = models.ForeignKey(
        "solicitudes.Solicitud", verbose_name="Solicitud", on_delete=models.CASCADE)

    edad = models.PositiveIntegerField(
        "Edad", blank=True, validators=[MinValueValidator(1)])

    calle = models.CharField('Calle', max_length=40)
    numero_exterior = models.PositiveIntegerField(
        "Número exterior", blank=True, validators=[MinValueValidator(1)])
    numero_interior = models.PositiveIntegerField(
        "Número interior", null=True, blank=True, validators=[MinValueValidator(1)])
    colonia = models.CharField("Colonia", max_length=35)
    codigo_postal = models.CharField(
        'Código Postal', max_length=5, validators=[MinLengthValidator(5),
                                                   solo_numeros])
    localidad = models.ForeignKey(
        "usuarios.Localidad", verbose_name='Localidad', on_delete=models.CASCADE)
    municipio = models.ForeignKey(
        "usuarios.Municipio", verbose_name='Municipio', on_delete=models.CASCADE)
    estado = models.ForeignKey(
        "usuarios.Estado", verbose_name='Estado', on_delete=models.CASCADE)
    entre_calles = models.CharField(
        "Entre calle y calle", max_length=60, blank=True, null=True)
    ...
    escolaridad = models.ForeignKey(
        Escolaridad, verbose_name="Escolaridad", on_delete=models.CASCADE)
    ocupacion = models.ForeignKey(
        Ocupacion, verbose_name="Ocupacion", on_delete=models.CASCADE)
    estado_civil = models.ForeignKey(
        EstadoCivil, verbose_name="Estado civil", on_delete=models.CASCADE)
    discapacidad = models.ForeignKey(
        Discapacidad, verbose_name="Discapacidad", on_delete=models.CASCADE)
    ...
    numero_recamaras = models.PositiveIntegerField(
        "Número de recamaras", validators=[MinValueValidator(0)])
    numero_banios = models.PositiveIntegerField(
        "Número de baños", validators=[MinValueValidator(0)])
    otros_caracteristicas_casa = models.CharField(
        "Otro", max_length=60, blank=True, null=True)

    piso_es = models.ForeignKey(
        Piso, verbose_name="El piso es", on_delete=models.CASCADE)
    techo_es = models.ForeignKey(
        Techo, verbose_name="El techo es", on_delete=models.CASCADE)
    automovil = models.ForeignKey(
        Automovil, verbose_name="Automóvil", on_delete=models.CASCADE)
    tipo_combustible = models.ForeignKey(
        TipoCombustible, verbose_name="Tipo de combustible", on_delete=models.CASCADE)

    casa_es = models.ForeignKey(
        CasaEs, verbose_name="Su casa es", on_delete=models.CASCADE)
    ...
    servicio_salud = models.ForeignKey(
        ServicioSalud, verbose_name="Servicio de salud", on_delete=models.CASCADE)
    ...
    enfermedad_hipertension = models.BooleanField(
        "Hipertensión", default=False)
    enfermedad_presion_baja = models.BooleanField(
        "Presión baja", default=False)
    enfermedad_discapacidad = models.BooleanField(
        "Discapacidad", default=False)
    ...
    enfermedad_paladar_hendido = models.BooleanField(
        "Paladar Hendido", default=False)
    enfermedad_cardiopatias = models.BooleanField(
        "Cardiopatías", default=False)
